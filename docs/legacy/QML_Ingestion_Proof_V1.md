...
> [!WARNING]
> **DOCUMENTO HISTÓRICO (V1)**
> Este documento descreve uma arquitetura aspiracional e inicial (V1) que previa um fallback heurístico de 3 níveis e processamento de diretório. 
> No **Atlas Cortex V2**, o motor foi consolidado estritamente no Nível 1 (Parsing Determinístico via AST com Tree-Sitter) para máxima previsibilidade matemática, eliminando as heurísticas implícitas.
> A matemática de complexidade ($O(N)$ e $O(\log V)$) listada neste documento continua válida, mas os detalhes da implementação do "Cascading Router" foram descontinuados.
